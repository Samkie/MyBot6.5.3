...
Func getArmyHeroCount($bOpenArmyWindow = False, $bCloseArmyWindow = False)
	...
	;	If $debugsetlogTrain = 1 Or $debugArmyHeroCount = 1 Then SetLog("$bFullArmyHero= " & $bFullArmyHero, $COLOR_DEBUG) ;Debug
		...
		For $i = $DB To $LB
			...
			EndIf
		Next
		If ($iDBcheck = 1 And BitAND($iHeroWait[$DB], $iHeroAvailable + $iHeroUpgradingBit) = $iHeroWait[$DB]) Or _
		   ($iABcheck = 1 And BitAND($iHeroWait[$LB], $iHeroAvailable + $iHeroUpgradingBit) = $iHeroWait[$LB]) Then
			$bFullArmyHero = True
		EndIf
		; =================

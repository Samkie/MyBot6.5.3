...
Func runBot() ;Bot that runs everything in order
	...
	While 1
		If $iSamM0dDebug And $Restart Then SetLog("Continue loop with restart", $COLOR_DEBUG)
...
Func Idle() ;Sequence that runs until Full Army
	...
	If $iSamM0dDebug Then SetLog("$fullArmy: " & $fullArmy & "   $bFullArmyHero: " & $bFullArmyHero & "   $bFullArmySpells: " & $bFullArmySpells)
	If $iSamM0dDebug Then SetLog("$IsFullArmywithHeroesAndSpells: " & $IsFullArmywithHeroesAndSpells)
	...
	While $IsFullArmywithHeroesAndSpells = False
		...
		If $bActiveDonate = True Then
			...
			If $iSkipDonateNearFulLTroopsEnable = 1 Then
				$aHeroResult = CheckArmyCamp(True, True, True)
			EndIf
...
Func AttackMain() ;Main control for attack functions
	...
	If IsSearchAttackEnabled() Then
		...
		Else
			...
			Setlog("Waiting on troops, CC Troops, heroes and/or spells according to search settings", $COLOR_WARNING)
...
Func QuickAttack()
	...
	Local $bFlag4IfNeedQuickAttack = ($iChkTrophyRange = 1 Or $iAtkAlgorithm[$DB] = 2 Or $iTScheck = 1) ; only following enable then i open army window for check getArmyCapacity()
